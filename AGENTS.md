...
### Components
- `hld/` - Go daemon that coordinates approvals and manages Codex sessions
- `hlyr/` - TypeScript CLI with MCP (Model Context Protocol) server for Codex integration
- `humanlayer-wui/` - CodeLayer - Desktop/Web UI (Tauri + React) for graphical approval management
- `Codex-go/` - Go SDK for programmatically launching Codex sessions

### Architecture Flow
```
Codex → MCP Protocol → hlyr → JSON-RPC → hld → HumanLayer Cloud API
                                         ↑         ↑
                                    TUI ─┘         └─ WUI
```
